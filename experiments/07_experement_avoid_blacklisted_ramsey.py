...
def fit_ramsey_robust(x_times, y_probs):
    ...
    # Bounds prevent the optimizer from wandering into "math magic" zones
    ...
    try:
        ...
        # If it still fails, return explicit failure rather than crashing
...
def generate_brutal_report(qubit_data):
    ...
    for q_id in sorted_ids:
        ...
        # CRITICAL FIX: Exclude "Detuned" qubits from global shift calculation
    ...
    # --- 3. GLOBAL CORRECTION CALCULATION ---
    print("-" * 60)
    if shifts:
...
if __name__ == "__main__":
    # ------------------------------------------------------------------
    # NOTE: Plug your data fetching logic here.
    # If using your existing lists (x, y) from the previous loop:
    # ------------------------------------------------------------------
    
    # Mock Data to demonstrate the fix on your specific problem cases
    print("Running Simulation with Q1 (Normal), Q7 (Shift Issue), and Q26 (Noise Issue)...")
    
    t_sim = np.linspace(0, 100e-6, 15) 
    
    # Qubit 1 Data Simulation: Normal (T2=190us, Shift=-5kHz)
    y_q1 = 0.5 * np.exp(-t_sim/190e-6) * np.cos(2*np.pi*-5e3*t_sim) + 0.5

    # Qubit 7 Data Simulation: Good T2 (312us), Bad Shift (57kHz)
    y_q7 = 0.5 * np.exp(-t_sim/312e-6) * np.cos(2*np.pi*57e3*t_sim) + 0.5
    
    # Qubit 26 Data Simulation: Bad T2 (37us), Bad Shift (-16kHz) + Heavy Noise
    np.random.seed(42)
    y_q26 = 0.5 * np.exp(-t_sim/37e-6) * np.cos(2*np.pi*-16.7e3*t_sim) + 0.5 + np.random.normal(0, 0.05, len(t_sim))
    
    # Perform Analysis
    results = {}
    
    print("Fitting Qubit 1 (Control)...")
    results[1] = fit_ramsey_robust(t_sim, y_q1)

    print("Fitting Qubit 7...")
    results[7] = fit_ramsey_robust(t_sim, y_q7)
    
    print("Fitting Qubit 26...")
    results[26] = fit_ramsey_robust(t_sim, y_q26)
